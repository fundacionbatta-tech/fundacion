...
const sidebarItems = [
  ...
  // Removed Subscriptions and Payments menu items
];
      ...
      <div className={'w-full'}>
        {sidebarItems.map((item) => (
          <Link
            key={item.title}
            href={item.href}
            className={cn('flex items-center text-base gap-3 px-4 py-3 rounded-xxs dashboard-sidebar-items text-[16px] leading-[24px]')}
          >
            {item.icon}
            {item.title}
          </Link>
